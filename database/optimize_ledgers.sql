-- Optimized customer ledger summary function for better performance
...
-- Enable trigram extension for better text search FIRST
...
-- Create trigram index AFTER enabling the extension
CREATE INDEX IF NOT EXISTS idx_customers_search ON customers USING gin((name || ' ' || COALESCE(email, '') || ' ' || COALESCE(phone, '')) gin_trgm_ops);
