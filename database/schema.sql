...
CREATE TABLE public.asset_categories (
  ...
  description text,
  depreciation_method character varying DEFAULT 'straight_line'::character varying CHECK (depreciation_method::text = ANY (ARRAY['straight_line'::character varying, 'declining_balance'::character varying, 'units_of_production'::character varying, 'no_depreciation'::character varying]::text[])),
  default_useful_life integer,
  default_salvage_percentage numeric DEFAULT 0,
  chart_account_code character varying,
...
CREATE TABLE public.asset_depreciation_schedule (
  ...
  depreciation_month integer NOT NULL CHECK (depreciation_month >= 1 AND depreciation_month <= 12),
  period_start_date date NOT NULL,
  period_end_date date NOT NULL,
  opening_book_value numeric NOT NULL,
  depreciation_amount numeric NOT NULL,
  accumulated_depreciation numeric NOT NULL,
  closing_book_value numeric NOT NULL,
  ...
  CONSTRAINT asset_depreciation_schedule_asset_id_fkey FOREIGN KEY (asset_id) REFERENCES public.assets(id),
  CONSTRAINT asset_depreciation_schedule_journal_entry_id_fkey FOREIGN KEY (journal_entry_id) REFERENCES public.journal_entries(id),
  CONSTRAINT asset_depreciation_schedule_posted_by_fkey FOREIGN KEY (posted_by) REFERENCES public.users(id)
);
CREATE TABLE public.asset_disposals (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  asset_id uuid NOT NULL,
  disposal_date date NOT NULL,
  disposal_type character varying NOT NULL CHECK (disposal_type::text = ANY (ARRAY['sale'::character varying, 'donation'::character varying, 'scrap'::character varying, 'trade_in'::character varying, 'theft'::character varying, 'destruction'::character varying, 'obsolete'::character varying]::text[])),
  original_cost numeric NOT NULL,
  accumulated_depreciation numeric NOT NULL,
  book_value numeric DEFAULT (original_cost - accumulated_depreciation),
  sale_price numeric DEFAULT 0,
  buyer_name character varying,
  buyer_contact character varying,
  sale_invoice_number character varying,
  gain_loss numeric DEFAULT (sale_price - (original_cost - accumulated_depreciation)),
  payment_method character varying CHECK (payment_method::text = ANY (ARRAY['cash'::character varying, 'bank_transfer'::character varying, 'cheque'::character varying, 'credit_card'::character varying, 'trade_in'::character varying]::text[])),
  payment_received numeric DEFAULT 0,
  bank_account_id uuid,
  approved_by uuid,
  approval_date date,
  reason text NOT NULL,
  disposal_certificate character varying,
  documents jsonb,
  created_by uuid,
  created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT asset_disposals_pkey PRIMARY KEY (id),
  CONSTRAINT asset_disposals_asset_id_fkey FOREIGN KEY (asset_id) REFERENCES public.assets(id),
  CONSTRAINT asset_disposals_bank_account_id_fkey FOREIGN KEY (bank_account_id) REFERENCES public.bank_accounts(id),
  CONSTRAINT asset_disposals_approved_by_fkey FOREIGN KEY (approved_by) REFERENCES public.users(id),
  CONSTRAINT asset_disposals_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id)
);
CREATE TABLE public.asset_maintenance_log (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  asset_id uuid NOT NULL,
  maintenance_date date NOT NULL,
  maintenance_type character varying CHECK (maintenance_type::text = ANY (ARRAY['routine'::character varying, 'preventive'::character varying, 'corrective'::character varying, 'emergency'::character varying, 'inspection'::character varying, 'upgrade'::character varying]::text[])),
  ...
  total_cost numeric DEFAULT ((labor_cost + parts_cost) + other_cost),
  status character varying DEFAULT 'completed'::character varying CHECK (status::text = ANY (ARRAY['scheduled'::character varying, 'in_progress'::character varying, 'completed'::character varying, 'cancelled'::character varying]::text[])),
  ...
  created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT asset_maintenance_log_pkey PRIMARY KEY (id),
  CONSTRAINT asset_maintenance_log_asset_id_fkey FOREIGN KEY (asset_id) REFERENCES public.assets(id),
  CONSTRAINT asset_maintenance_log_vendor_id_fkey FOREIGN KEY (vendor_id) REFERENCES public.suppliers(id),
  CONSTRAINT asset_maintenance_log_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id)
);
CREATE TABLE public.asset_purchases (
  ...
  asset_cost numeric NOT NULL CHECK (asset_cost > 0::numeric),
  installation_cost numeric DEFAULT 0,
  freight_cost numeric DEFAULT 0,
  other_costs numeric DEFAULT 0,
  total_cost numeric DEFAULT (((asset_cost + installation_cost) + freight_cost) + other_costs),
  payment_method character varying CHECK (payment_method::text = ANY (ARRAY['cash'::character varying, 'bank_transfer'::character varying, 'cheque'::character varying, 'credit_card'::character varying, 'loan'::character varying, 'lease'::character varying]::text[])),
  payment_status character varying DEFAULT 'pending'::character varying CHECK (payment_status::text = ANY (ARRAY['pending'::character varying, 'partial'::character varying, 'paid'::character varying, 'financed'::character varying]::text[])),
  amount_paid numeric DEFAULT 0,
  balance_due numeric DEFAULT ((((asset_cost + installation_cost) + freight_cost) + other_costs) - amount_paid),
  bank_account_id uuid,
  loan_id uuid,
  documents jsonb,
  notes text,
  ...
  CONSTRAINT asset_purchases_asset_id_fkey FOREIGN KEY (asset_id) REFERENCES public.assets(id),
  CONSTRAINT asset_purchases_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES public.suppliers(id),
  CONSTRAINT asset_purchases_bank_account_id_fkey FOREIGN KEY (bank_account_id) REFERENCES public.bank_accounts(id),
  CONSTRAINT asset_purchases_loan_id_fkey FOREIGN KEY (loan_id) REFERENCES public.loan_opening_balances(id),
  CONSTRAINT asset_purchases_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id)
);
CREATE TABLE public.assets (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  asset_tag character varying NOT NULL UNIQUE,
  ...
  purchase_reference character varying,
  depreciation_method character varying DEFAULT 'straight_line'::character varying CHECK (depreciation_method::text = ANY (ARRAY['straight_line'::character varying, 'declining_balance'::character varying, 'units_of_production'::character varying, 'no_depreciation'::character varying]::text[])),
  ...
  status character varying DEFAULT 'active'::character varying CHECK (status::text = ANY (ARRAY['active'::character varying, 'under_maintenance'::character varying, 'retired'::character varying, 'disposed'::character varying, 'sold'::character varying, 'donated'::character varying, 'scrapped'::character varying]::text[])),
  ...
  asset_account_code character varying,
  depreciation_account_code character varying,
  accumulated_depreciation_account_code character varying,
  notes text,
  created_by uuid,
  created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  ...
  CONSTRAINT assets_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES public.suppliers(id),
  CONSTRAINT assets_assigned_to_fkey FOREIGN KEY (assigned_to) REFERENCES public.employees(id),
  CONSTRAINT assets_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.employee_obligations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  employee_id uuid NOT NULL,
  obligation_date date NOT NULL,
  obligation_type character varying NOT NULL CHECK (obligation_type::text = ANY (ARRAY['salary'::character varying, 'incentive'::character varying, 'overtime'::character varying, 'bonus'::character varying, 'allowance'::character varying, 'commission'::character varying, 'other'::character varying]::text[])),
  ...
  updated_at timestamp without time zone DEFAULT now(),
  updated_by uuid,
  CONSTRAINT employee_obligations_pkey PRIMARY KEY (id),
  CONSTRAINT employee_obligations_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employees(id),
  CONSTRAINT employee_obligations_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id),
  CONSTRAINT employee_obligations_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.expenses (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  date date NOT NULL,
  category text NOT NULL CHECK (category = ANY (ARRAY['Raw Materials'::text, 'Direct Labor'::text, 'Manufacturing Overhead'::text, 'Administrative'::text, 'Salaries & Benefits'::text, 'Marketing & Sales'::text, 'Logistics & Distribution'::text, 'Technology'::text, 'Insurance'::text, 'Maintenance & Repairs'::text, 'Travel & Entertainment'::text, 'Research & Development'::text, 'Miscellaneous'::text, 'Vehicle Fleet'::text, 'Accounts Payable'::text, 'Prepaid Expenses'::text, 'Rent'::text, 'Utilities'::text, 'Salaries'::text, 'Marketing'::text, 'Other'::text, 'Manufacturing'::text, 'Production'::text, 'Maintenance'::text, 'Administration'::text, 'Logistics'::text, 'Customer Refunds'::text, 'Capital Expenditure'::text, 'Asset Purchase'::text, 'Equipment Purchase'::text, 'Vehicle Purchase'::text, 'Property Purchase'::text, 'Building Purchase'::text, 'Machinery Purchase'::text, 'Furniture Purchase'::text, 'Computer Equipment Purchase'::text, 'Software Purchase'::text, 'Asset Improvement'::text, 'Asset Installation'::text])),
  description text,
  amount numeric NOT NULL,
  payment_method text,
  created_by uuid,
  ...
  subcategory text,
  entity_type text CHECK (entity_type = ANY (ARRAY['truck'::text, 'employee'::text, 'supplier'::text, 'customer'::text])),
  entity_id uuid,
...
CREATE TABLE public.invoice_refunds (
  ...
  bank_account_id uuid,
  reference_number character varying,
  status character varying NOT NULL DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying::text, 'approved'::character varying::text, 'processed'::character varying::text, 'rejected'::character varying::text, 'cancelled'::character varying::text, 'reversed'::character varying::text])),
  requested_by uuid NOT NULL,
  approved_by uuid,
  processed_by uuid,
  approved_at timestamp without time zone,
...
CREATE TABLE public.meta_ads (
  ...
  name text NOT NULL,
  status text DEFAULT 'ACTIVE'::text CHECK (status = ANY (ARRAY['ACTIVE'::text, 'PAUSED'::text, 'ARCHIVED'::text, 'DELETED'::text])),
  creative_id text,
  ad_copy text,
  headline text,
  description text,
...
CREATE TABLE public.meta_campaigns (
  ...
  objective text,
  status text DEFAULT 'ACTIVE'::text CHECK (status = ANY (ARRAY['ACTIVE'::text, 'PAUSED'::text, 'ARCHIVED'::text, 'DELETED'::text])),
  platform text DEFAULT 'multi'::text CHECK (platform = ANY (ARRAY['facebook'::text, 'instagram'::text, 'whatsapp'::text, 'multi'::text])),
  ...
  updated_at timestamp without time zone DEFAULT now(),
  last_synced_at timestamp without time zone,
  sync_status text DEFAULT 'pending'::text CHECK (sync_status = ANY (ARRAY['pending'::text, 'syncing'::text, 'synced'::text, 'error'::text])),
  sync_error text,
  meta_data jsonb,
...
CREATE TABLE public.meta_leads (
  ...
  form_id text,
  form_name text,
  platform text NOT NULL CHECK (platform = ANY (ARRAY['facebook'::text, 'instagram'::text, 'whatsapp'::text])),
  status text DEFAULT 'new'::text CHECK (status = ANY (ARRAY['new'::text, 'contacted'::text, 'qualified'::text, 'converted'::text, 'rejected'::text, 'spam'::text])),
  assigned_to uuid,
  ...
  CONSTRAINT meta_leads_campaign_id_fkey FOREIGN KEY (campaign_id) REFERENCES public.meta_campaigns(id),
  CONSTRAINT meta_leads_adset_id_fkey FOREIGN KEY (adset_id) REFERENCES public.meta_adsets(id),
  CONSTRAINT meta_leads_ad_id_fkey FOREIGN KEY (ad_id) REFERENCES public.meta_ads(id),
  CONSTRAINT meta_leads_assigned_to_fkey FOREIGN KEY (assigned_to) REFERENCES public.users(id),
  CONSTRAINT meta_leads_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES public.customers(id)
);
CREATE TABLE public.meta_sync_log (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  sync_type text NOT NULL CHECK (sync_type = ANY (ARRAY['campaigns'::text, 'adsets'::text, 'ads'::text, 'leads'::text, 'full'::text])),
  ...
  CONSTRAINT meta_sync_log_triggered_by_fkey FOREIGN KEY (triggered_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.opening_stock_snapshots (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  snapshot_date date NOT NULL UNIQUE,
  opening_stock_value numeric NOT NULL DEFAULT 0,
  closing_stock_value numeric NOT NULL DEFAULT 0,
  total_sales_cost numeric NOT NULL DEFAULT 0,
  snapshot_type character varying NOT NULL DEFAULT 'month_end'::character varying,
  period_label character varying,
  calculated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  notes text,
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT opening_stock_snapshots_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.purchase_return_documents (
  ...
  document_type text NOT NULL CHECK (document_type = ANY (ARRAY['return_receipt'::text, 'quality_report'::text, 'photo'::text, 'vendor_agreement'::text, 'other'::text])),
  ...
  uploaded_by uuid,
  uploaded_at timestamp with time zone DEFAULT now(),
  CONSTRAINT purchase_return_documents_pkey PRIMARY KEY (id),
  CONSTRAINT purchase_return_documents_purchase_return_id_fkey FOREIGN KEY (purchase_return_id) REFERENCES public.purchase_returns(id),
  CONSTRAINT purchase_return_documents_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES public.users(id)
);
CREATE TABLE public.purchase_return_line_items (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  purchase_return_id uuid NOT NULL,
  vendor_bill_line_item_id uuid NOT NULL,
  product_id uuid,
  product_name text NOT NULL,
  description text,
  ...
  actual_cost_per_unit numeric CHECK (actual_cost_per_unit >= 0::numeric),
  line_return_total numeric NOT NULL DEFAULT 0 CHECK (line_return_total >= 0::numeric),
  line_gst_amount numeric DEFAULT 0 CHECK (line_gst_amount >= 0::numeric),
  return_reason text NOT NULL CHECK (return_reason = ANY (ARRAY['defective'::text, 'wrong_item'::text, 'damaged'::text, 'excess_quantity'::text, 'quality_issue'::text, 'other'::text])),
  condition_at_return text CHECK (condition_at_return = ANY (ARRAY['unopened'::text, 'opened_unused'::text, 'partially_used'::text, 'damaged'::text, 'defective'::text])),
  is_restockable boolean DEFAULT false,
  quality_check_passed boolean,
  quality_check_notes text,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT purchase_return_line_items_pkey PRIMARY KEY (id),
  CONSTRAINT purchase_return_line_items_purchase_return_id_fkey FOREIGN KEY (purchase_return_id) REFERENCES public.purchase_returns(id),
  CONSTRAINT purchase_return_line_items_vendor_bill_line_item_id_fkey FOREIGN KEY (vendor_bill_line_item_id) REFERENCES public.vendor_bill_line_items(id),
  CONSTRAINT purchase_return_line_items_product_id_fkey FOREIGN KEY (product_id) REFERENCES public.products(id)
);
CREATE TABLE public.purchase_return_payment_reversals (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  purchase_return_id uuid NOT NULL,
  original_vendor_payment_id uuid,
  original_expense_id uuid,
  reversed_amount numeric NOT NULL CHECK (reversed_amount > 0::numeric),
  reversal_method text NOT NULL CHECK (reversal_method = ANY (ARRAY['cash'::text, 'bank_transfer'::text, 'cheque'::text, 'upi'::text, 'card'::text, 'credit_note'::text])),
  reversal_date date NOT NULL DEFAULT CURRENT_DATE,
  credited_bank_account_id uuid,
  reversal_reference_number text,
  bank_transaction_id uuid,
  journal_entry_id uuid,
  expense_reversal_id uuid,
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'completed'::text, 'failed'::text])),
  reversal_notes text,
  processed_by uuid,
  processed_at timestamp with time zone,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT purchase_return_payment_reversals_pkey PRIMARY KEY (id),
  CONSTRAINT purchase_return_payment_reversals_purchase_return_id_fkey FOREIGN KEY (purchase_return_id) REFERENCES public.purchase_returns(id),
  CONSTRAINT purchase_return_payment_reversals_original_vendor_payment_id_fk FOREIGN KEY (original_vendor_payment_id) REFERENCES public.vendor_payment_history(id),
  CONSTRAINT purchase_return_payment_reversals_original_expense_id_fkey FOREIGN KEY (original_expense_id) REFERENCES public.expenses(id),
  CONSTRAINT purchase_return_payment_reversals_credited_bank_account_id_fkey FOREIGN KEY (credited_bank_account_id) REFERENCES public.bank_accounts(id),
  CONSTRAINT purchase_return_payment_reversals_bank_transaction_id_fkey FOREIGN KEY (bank_transaction_id) REFERENCES public.bank_transactions(id),
  CONSTRAINT purchase_return_payment_reversals_expense_reversal_id_fkey FOREIGN KEY (expense_reversal_id) REFERENCES public.expenses(id),
  CONSTRAINT purchase_return_payment_reversals_processed_by_fkey FOREIGN KEY (processed_by) REFERENCES public.users(id)
);
CREATE TABLE public.purchase_returns (
  ...
  total_return_amount numeric NOT NULL DEFAULT 0 CHECK (total_return_amount >= 0::numeric),
  returned_gst_amount numeric DEFAULT 0 CHECK (returned_gst_amount >= 0::numeric),
  net_return_amount numeric NOT NULL DEFAULT 0 CHECK (net_return_amount >= 0::numeric),
  reason text NOT NULL CHECK (reason = ANY (ARRAY['defective'::text, 'wrong_item'::text, 'damaged'::text, 'excess_quantity'::text, 'quality_issue'::text, 'other'::text])),
  reason_description text,
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text, 'processed'::text, 'payment_reversed'::text])),
  is_payment_reversed boolean DEFAULT false,
  reversal_processed_at timestamp with time zone,
  reversal_method text CHECK (reversal_method = ANY (ARRAY['cash'::text, 'bank_transfer'::text, 'cheque'::text, 'upi'::text, 'card'::text, 'credit_note'::text])),
  reversal_bank_account_id uuid,
  reversal_reference_number text,
  created_by uuid,
  approved_by uuid,
  processed_by uuid,
  ...
  CONSTRAINT purchase_returns_vendor_bill_id_fkey FOREIGN KEY (vendor_bill_id) REFERENCES public.vendor_bills(id),
  CONSTRAINT purchase_returns_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES public.suppliers(id),
  CONSTRAINT purchase_returns_reversal_bank_account_id_fkey FOREIGN KEY (reversal_bank_account_id) REFERENCES public.bank_accounts(id),
  CONSTRAINT purchase_returns_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id),
  CONSTRAINT purchase_returns_approved_by_fkey FOREIGN KEY (approved_by) REFERENCES public.users(id),
  CONSTRAINT purchase_returns_processed_by_fkey FOREIGN KEY (processed_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.vendor_bill_line_items (
  ...
  unit_price numeric NOT NULL CHECK (unit_price >= 0::numeric),
  actual_cost_per_unit numeric DEFAULT NULL::numeric CHECK (actual_cost_per_unit >= 0::numeric),
  purchase_order_id uuid,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  total_returned_quantity integer DEFAULT 0 CHECK (total_returned_quantity >= 0),
  remaining_quantity integer DEFAULT (quantity - COALESCE(total_returned_quantity, 0)),
  is_fully_returned boolean DEFAULT (total_returned_quantity >= quantity),
  CONSTRAINT vendor_bill_line_items_pkey PRIMARY KEY (id),
  CONSTRAINT vendor_bill_line_items_vendor_bill_id_fkey FOREIGN KEY (vendor_bill_id) REFERENCES public.vendor_bills(id)
);
...
CREATE TABLE public.vendor_bills (
  ...
  total_returned_amount numeric DEFAULT 0 CHECK (total_returned_amount >= 0::numeric),
  net_bill_after_returns numeric DEFAULT (total_amount - COALESCE(total_returned_amount, (0)::numeric)),
