...
CREATE TABLE public.expenses (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  date date NOT NULL,
  category text NOT NULL CHECK (category = ANY (ARRAY['Raw Materials'::text, 'Direct Labor'::text, 'Manufacturing Overhead'::text, 'Administrative'::text, 'Salaries & Benefits'::text, 'Marketing & Sales'::text, 'Logistics & Distribution'::text, 'Technology'::text, 'Insurance'::text, 'Maintenance & Repairs'::text, 'Travel & Entertainment'::text, 'Research & Development'::text, 'Miscellaneous'::text, 'Vehicle Fleet'::text, 'Accounts Payable'::text, 'Prepaid Expenses'::text, 'Rent'::text, 'Utilities'::text, 'Salaries'::text, 'Marketing'::text, 'Other'::text, 'Manufacturing'::text, 'Production'::text, 'Maintenance'::text, 'Administration'::text, 'Logistics'::text, 'Customer Refunds'::text])),
  description text,
  amount numeric NOT NULL,
  payment_method text,
  created_by uuid,
