...
const AlertItem = ({ alert }) => {
    ...
    // Fallback in case an icon isn't provided
    const Icon = alert.icon ? alert.icon : () => '🔔';
        ...
        <div className={`flex items-start gap-4 p-4 rounded-lg border ${priorityStyles[alert.priority] || 'bg-gray-100 border-gray-200'}`}>
            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    ...
                    <Badge className={`${badgeStyles[alert.priority] || 'bg-gray-500'} text-white`}>{alert.priority}</Badge>
                </div>
