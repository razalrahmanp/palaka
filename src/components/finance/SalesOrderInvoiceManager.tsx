...
export function SalesOrderInvoiceManager() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('Sales Orders Data:', ordersData); // Debugging line
      const invoicesData = await invoicesRes.json();
      const paymentsData = await paymentsRes.json();

      // Process orders with invoice status
      const processedOrders = ordersData.map((order: SalesOrder) => {
        const orderInvoices = invoicesData.filter((inv: Invoice) => inv.sales_order_id === order.id);
        const totalInvoiced = orderInvoices.reduce((sum: number, inv: Invoice) => sum + inv.total, 0);
        const remainingToInvoice = (order.final_price || order.total_price) - totalInvoiced;
        
        // Calculate paid amount for each order
        const orderPayments = paymentsData.filter((payment: PaymentDetails) => payment.sales_order_id === order.id);
  ...
  const handleSendWhatsApp = async (order: SalesOrderWithInvoice) => {
    try {
      ...
      const result = await WhatsAppService.sendBill(orderDetails.customer_phone, billData);
  ...
  const totalOrderValue = salesOrders.reduce((sum, order) => sum + (order.final_price || order.total_price), 0);
          ...
          <div className="bg-white">
            ...
            <TabsContent value="orders" className="space-y-4 p-6">
              ...
              <Tabs defaultValue="invoice-orders">
                <TabsList>
                  <TabsTrigger value="invoice-orders">All Sales Orders</TabsTrigger>
                  <TabsTrigger value="payment-orders">All Sales Orders & Payments</TabsTrigger>
                </TabsList>
                
                <TabsContent value="invoice-orders" className="pt-4">
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-sm text-gray-600">
                      Showing all sales orders. Filter options available for better navigation.
                    </p>
                  </div>
                        ...
                        {salesOrders.map((order) => (
                          <TableRow key={order.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <TableCell className="font-semibold text-gray-900">
                              {formatCurrency(order.final_price || order.total_price)}
                            </TableCell>
                            <TableCell>
                              {getInvoiceStatusBadge(order.invoice_status || 'not_invoiced')}
                            </TableCell>
                            <TableCell className="text-green-600 font-medium">
                              {formatCurrency(order.total_invoiced || 0)}
                            </TableCell>
                            <TableCell className="text-orange-600 font-medium">
                              {formatCurrency(order.remaining_to_invoice || 0)}
                            </TableCell>
                            <TableCell>
                              {getPaymentStatusBadge(order.payment_status || 
                                ((order.paid_amount || 0) >= (order.final_price || order.total_price) ? 'PAID' : 
                            ...
                            <span className="ml-1">
                              <SalesOrderPaymentTracker
                                orderId={order.id}
                                orderTotal={order.final_price || order.total_price}
                                onPaymentAdded={() => {
                                  fetchData();
                      ...
                      <TableBody>
                        {salesOrders.map((order) => (
                          <TableRow key={order.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <TableCell className="text-orange-600 font-medium">
                              {formatCurrency((order.final_price || order.total_price) - (order.paid_amount || 0))}
                            </TableCell>
                            <TableCell>
                              {getPaymentStatusBadge(order.payment_status || 
                                ((order.paid_amount || 0) >= (order.final_price || order.total_price) ? 'PAID' : 
